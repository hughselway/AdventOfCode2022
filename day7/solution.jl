...
struct Directory_
    name::String
    parent_directory::Union{Directory_,Nothing}
    files::Vector{Tuple{String, Int}}
    child_directories::Vector{Directory_}
end

function parse_data(data)::Tuple{Directory_,Vector{Directory_}}
    root_directory = Directory_("/", nothing,[], [])
    current_directory = root_directory
    directory_list::Vector{Directory_} = []
    for line in data[2:end]
        ...
        if line[1:4] == "\$ cd"
            ...
            current_directory = current_directory.child_directories[
                findfirst(d -> d.name == line[6:end],
                current_directory.child_directories)]
        elseif line[1:4] == "\$ ls"
            continue
        elseif line[1:3] == "dir"
            push!(current_directory.child_directories,
                Directory_(line[5:end], current_directory, [], []))
...
function calculate_directory_size(directory::Directory_)
    size = 0
...
function task2(example::Bool=false)::Int
    ...
    for directory in directory_list
        size = calculate_directory_size(directory)
        if (size > space_needed && current_smallest_sufficient_directory_size === nothing)|| (space_needed < size < current_smallest_sufficient_directory_size)
