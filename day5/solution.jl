...
function get_arrangement_and_instruction(
    example::Bool = false,
...
function parse_arrangement_to_stacks(arrangement::Vector{String})::Stacks
    ...
    for line in reverse(arrangement)[2:end]
        for (stack_index, stack_value) in
            enumerate((line[4x-3:4x-1] for x in 1:number_of_stacks))
            if stack_value == "   "
                continue
            end
...
function task1(example::Bool = false)::String
    ...
    for line in instruction
        ...
        push!(
            ...
            (pop!(stacks[from_stack_index]) for _ in 1:number_to_move)...,
        )
    end
