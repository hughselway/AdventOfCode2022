...
function update_visibility(tree_heights::Array{Int64, 2}, visible_points::Array{Bool,2}, side_length::Int, reverse::Bool, vertical::Bool)::Array{Bool,2}
    for i in 1:side_length
        ...
        for j in (reverse ? (side_length:-1:1) : (1:side_length))
            ...
            if (tree_heights[coord_1, coord_2] > current_height) || (j == (reverse ? side_length : 1))
                # print("X")
...
function task1(example::Bool=false)
    ...
    for vertical in [false, true]
        for reverse in [false, true]
            ...
            visible_points = update_visibility(tree_heights, visible_points, side_length,reverse, vertical)
        end
    end
...
function viewing_distance(i, j, tree_heights, side_length, vertical, reverse)
    ...
    for k in (reverse ? (starting_value-1:-1:1) : (starting_value+1:side_length))
...
function scenic_score(i, j, tree_heights, side_length)
    ...
    for vertical in (false, true)
        for reverse in (false, true)
            ...
            score *= viewing_distance(i, j, tree_heights, side_length, vertical, reverse)
        end
    end
    return score
end
